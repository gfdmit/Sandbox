...
template <typename T>
inline stack<T>::stack(std::initializer_list<value_type> const &items) noexcept : list_() {
...
template <typename T>
inline stack<T>::const_reference stack<T>::top() const {
    if (list_.front() == T()) {
...
template <typename T>
inline stack<T>::size_type stack<T>::size() const noexcept {
    list_.size();
}
